...
public class Add_Update_User extends Activity implements OnItemSelectedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		categories.add("");
		categories.add("Τρόφιμα");
		categories.add("Ποτά");
		categories.add("Αναψυκτικά");
		categories.add("Καφέδες");
		categories.add("Υλικά");
		...
		add_save_btn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (valid_name != null && valid_epitheto != null
						...
						&& valid_eponimia_user != null && valid_afm != null
						&& valid_name.length() != 0
					...
					Intent add_user = new Intent(Add_Update_User.this,
							Promitheutis.class);
		...
		update_btn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// check the value state is null or not
				if (valid_name != null && valid_epitheto != null
						&& valid_mob_number != null && valid_name.length() != 0
						&& valid_epitheto.length() != 0
						&& valid_mob_number.length() != 0
						&& valid_eidos != null && valid_eidos.length() != 0
	...
	public void Is_Valid_Sign_Number_Validation(int MinLen, int MaxLen,
		...
		if (edt.getText().toString().length() <= 0) {
			edt.setError(Html.fromHtml("<font color='red'>αριθμούς μόνο</font>"));
			//edt.setError("Αριθμούς μόνο");
			valid_mob_number = null;
		} else if (edt.getText().toString().length() < MinLen) {
			edt.setError(Html.fromHtml("<font color='red'>ελάχιστοι χαρακτήρες 10</font>"));
			//edt.setError("Ελάχιστοι χαρακτήρες " + MinLen);
			valid_mob_number = null;

		} else if (edt.getText().toString().length() > MaxLen) {
			edt.setError(Html.fromHtml("<font color='red'>μέγιστοι χαρακτήρες 10</font>"));
	...
	public void Is_Valid_Afm(int MinLen, int MaxLen, EditText edt)
		...
		if (edt.getText().toString().length() <= 0) {
			...
			edt.setError(Html.fromHtml("<font color='red'>ελάχιστοι χαρακτήρες 9</font>"));
			//edt.setError("Ελάχιστοι χαρακτήρες " + MinLen);
			valid_afm = null;

		} else if (edt.getText().toString().length() > MaxLen) {
			edt.setError(Html.fromHtml("<font color='red'>μέγιστοι χαρακτήρες 9</font>"));
			//edt.setError("Μέγιστοι χαρακτήρες " + MaxLen);
			valid_afm = null;
	...
	public void Is_Valid_Email(EditText edt) {
		if (edt.getText().toString() == null) {
			edt.setError(Html.fromHtml("<font color='red'>άκυρη διεύθυνση email</font>"));
	...
	public void Is_Valid_Person_Name(EditText edt) throws NumberFormatException {
		if (edt.getText().toString().length() <= 0) {
			edt.setError(Html.fromHtml("<font color='red'>εισάγεται όνομα</font>"));
			//edt.setError("Εισάγεται όνομα");
			valid_name = null;
		} else if (!edt.getText().toString().matches("[a-zA-Zα-ωΑ-Ω ]+")) {
			edt.setError(Html.fromHtml("<font color='red'>εισάγεται χαρακτήρες</font>"));
	...
	public void Is_Valid_Eponimia_User(EditText edt)
		...
		if (edt.getText().toString().length() <= 0) {
			...
			//edt.setError("Εισάγεται όνομα");
			valid_eponimia_user = null;
		} else if (!edt.getText().toString().matches("[a-zA-Zα-ωΑ-Ω0-9 ]+")) {
			edt.setError(Html.fromHtml("<font color='red'>εισάγεται χαρακτήρες</font>"));
			//edt.setError("Εισάγεται χαρακτήρες");
			valid_eponimia_user = null;
		} else {
	...
	public void Is_Valid_Eidos(EditText edt) throws NumberFormatException {
		/*
		 * if (edt.getText().toString().length() <= 0) {
		 * edt.setError("Εισάγεται είδος"); valid_eidos = null; } else if
		 * (!edt.getText().toString().matches("[a-zA-Zα-ωΑ-Ω  ]+")) {
		 * //edt.setError("Εισάγεται χαρακτήρες"); valid_eidos = null; } else {
		 */
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		switch (parent.getId()) {
		case R.id.spinner:
			add_eidos.setText(item);
			valid_eidos = add_eidos.getText().toString();
			break;
		case R.id.spinner2:
			add_eidos2.setText(item);
			valid_eidos2 = add_eidos2.getText().toString();
			break;
		case R.id.spinner3:
			add_eidos3.setText(item);
			valid_eidos3 = add_eidos3.getText().toString();
			break;
		}
